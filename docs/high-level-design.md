...
| ID | Title | Actor | Flow |
|---|---|---|---|
| **UC-1** | Register a new domain | Domain owner | Owner runs `ddns register example`; the CLI performs commit-reveal under the hood — it commits to a salted hash of the name, waits out `MIN_COMMITMENT_AGE`, then reveals via `register()`; the contract checks the commitment, verifies the name is free and the fee is paid; on success the registry maps `example` to the owner's address and public key. Commit-reveal (mirroring ENS) is what stops an attacker who sees the reveal transaction in the mempool from front-running it: they cannot produce a matching commitment without knowing the secret salt, and cannot construct one in time even if they could, since a commitment must be at least `MIN_COMMITMENT_AGE` old before it can be revealed. |
